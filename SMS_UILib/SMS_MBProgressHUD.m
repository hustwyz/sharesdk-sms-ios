...
+ (SMS_MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	SMS_MBProgressHUD *hud = [[SMS_MBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[SMS_MBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		SMS_MBProgressHUD *HUD = (SMS_MBProgressHUD *)viewToRemove;
